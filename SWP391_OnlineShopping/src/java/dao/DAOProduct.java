...
//    }
    ...
    public List<Products> searchByName(String key, int index) {
        List<Products> list = new ArrayList<>();
        String sql = "WITH pd_ranked AS (\n"
                + "    SELECT pd.*, \n"
                + "           ROW_NUMBER() OVER (PARTITION BY pd.productID ORDER BY pd.pdetailID) AS rn\n"
                + "    FROM product_detail pd\n"
                + ")\n"
                + "SELECT p.*, pd.*\n"
                + "FROM onlineshopping.products p\n"
                + "INNER JOIN pd_ranked pd ON p.productID = pd.productID\n"
                + "WHERE p.productName LIKE ? AND pd.rn = 1;";
        try {
            conn = new DBContext().getConnection();
            ps = conn.prepareStatement(sql);
            ps.setString(1, "%" + key + "%");
//            ps.setInt(2, index * 8);
    ...
    public String getLatestProductID() {
        String sql = "SELECT productID FROM onlineshopping.products ORDER BY productID DESC LIMIT 1";
        ...
        try {
            ...
            rs = ps.executeQuery();
            if (rs.next()) {
                latestAccountID = rs.getString("productID");
            }
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
    ...
    public void insertDataProductDetail(String sizeId, String colorId, String quantity, String id) {
        try {
            conn = new DBContext().getConnection();
            String sql = "INSERT INTO product_detail (sizeID, colorID, quantity, productID) "
                    + "VALUES (?, ?, ?, ?)";
            ps = conn.prepareStatement(sql);
    ...
    public void insertDataProductImage(String colorId, String id, String image) {
        try {
            conn = new DBContext().getConnection();
            String sql = "INSERT INTO product_image (productID, colorID, imamge) "
                    + "VALUES (?, ?, ?)";
            ps = conn.prepareStatement(sql);
            ps.setString(1, id);
            ps.setString(2, colorId);
