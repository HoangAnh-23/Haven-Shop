    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (a != null) {
            if (a.getRoleID() == 5) {
                ...
                HttpSession session = request.getSession();
                session.setAttribute("acc", a);
                if(session.getAttribute("memoLink") != null) {
                    String linkDirect = (String)session.getAttribute("memoLink");
                    session.removeAttribute("memoLink");
                    response.sendRedirect(linkDirect);
                } else {
